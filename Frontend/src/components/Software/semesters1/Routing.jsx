...
function Student1() {
  // Initialize state with sessionStorage data or default value
  ...
  const sections = [
    ...
    },
    {
      title: 'PF',
      pdfs: [
        "https://drive.google.com/uc?id=15T3iF8pGI5pWoXXUWEcZCFXXM_iRzL-u&export=download",
        "https://drive.google.com/file/d/your-file-id-4/preview",
        // Add more PDF URLs
      ],
      isDownload: false,
    },
  ...
  return (
    <div>
      ...
      <main className="px-4 py-8 max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-center mb-8 text-[#692f95]">Student Material</h1>
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {sections.map((section, index) => (
            <div
              key={index}
              className={`bg-[#692f95] border border-[#692f95] rounded-lg shadow-md p-4 transition-colors duration-300 hover:bg-[#8c4c9f] ${activeSection === section.title ? 'border-[#37b350]' : ''}`}
                  ...
                  >
                    {section.isDownload ? (
                      ...
                      <a
                        href={pdfUrl}
                        download
                        className="block text-[#37b350] hover:text-white"
                      >
